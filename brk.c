...
do_brk(void *addr, void **ret)
{
        ...
        if ((unsigned int)addr > USER_MEM_HIGH || addr < curproc->p_start_brk){
                dbg(DBG_PRINT, "(GRADING3B)\n");
	...
	uint32_t end = ((uint32_t)addr-1)/PAGE_SIZE+1;
        ...
        if (start < end){
                dbg(DBG_PRINT, "(GRADING3B)\n");
                vmarea_t *vma = vmmap_lookup(map, start-1);
                int empty_val = vmmap_is_range_empty(map, start, (end - start));
                ...
                vmmap_remove(map, end, start-end);
                dbg(DBG_PRINT, "(GRADING3B)\n");
        }
