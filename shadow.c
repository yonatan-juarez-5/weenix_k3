...
shadow_create()
{
        mmobj_t *shdw = (mmobj_t*)slab_obj_alloc(shadow_allocator);
        mmobj_init(shdw, &shadow_mmobj_ops);
        shdw->mmo_refcount = 1;
...
shadow_lookuppage(mmobj_t *o, uint32_t pagenum, int forwrite, pframe_t **pf)
{
        ...
        if (forwrite == 1){
                p = pframe_get_resident(o, pagenum);
                ...
                if (p != NULL){
                        *pf = p;
                        ...
                        int get = pframe_get(o, pagenum, pf);
                        if (get == 0){
                ...
                while(shdw->mmo_shadowed != NULL){
                        p = pframe_get_resident(shdw, pagenum);
                        if (p == NULL){
                                shdw = shdw->mmo_shadowed;
                                dbg(DBG_PRINT, "(GRADING3B)\n");
                        }
                        else{
                                *pf = p;
                ...
                int lookup_res = pframe_lookup(shdw, pagenum, 0, pf);
                if (lookup_res == 0){
                        /* on return, (*pf) must be non-NULL */
                        KASSERT(NULL != (*pf)); 
                        dbg(DBG_PRINT, "(GRADING3A 6.d)\n");
...
shadow_fillpage(mmobj_t *o, pframe_t *pf)
{
        ...
        while(shdw->mmo_shadowed != NULL){
                p = pframe_get_resident(shdw->mmo_shadowed, pf->pf_pagenum);
                dbg(DBG_PRINT, "(GRADING3B)\n");
        ...
        int lookup_res = pframe_lookup(o->mmo_un.mmo_bottom_obj, pf->pf_pagenum, 0, &p);
        if (lookup_res == 0){
                pframe_pin(pf);
                memcpy(pf->pf_addr, p->pf_addr, PAGE_SIZE);
                dbg(DBG_PRINT, "(GRADING3B)\n");
...
{
        pframe_t *p = NULL;
        o->mmo_ops->lookuppage(o, pf->pf_pagenum, 1, &p);
