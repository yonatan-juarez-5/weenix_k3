...
{
        read_args_t r_args;
        int res = copy_from_user(&r_args, arg, sizeof(r_args));
        ...
        void *buf = page_alloc();
        int bytes = do_read(r_args.fd, buf, r_args.nbytes);
        ...
        res = copy_to_user(arg->buf, buf, bytes);
        if (res < 0){
                curthr->kt_errno = -res;
                page_free(buf);
                dbg(DBG_PRINT, "(GRADING3A)\n");
...
{
        write_args_t w_args;
        int res = copy_from_user(&w_args, arg, sizeof(w_args));
        if (res < 0){
                curthr->kt_errno = -res;
                dbg(DBG_PRINT, "(GRADING3A)\n");
                return -1;
        ...
        void *buf = page_alloc();
        res = copy_from_user(buf, w_args.buf, w_args.nbytes);
        if (res < 0){
                curthr->kt_errno = -res;
                page_free(buf);
                dbg(DBG_PRINT, "(GRADING3A)\n");
        ...
        int bytes = do_write(w_args.fd, buf, w_args.nbytes);
        if (bytes < 0){
                curthr->kt_errno = -res;
                page_free(buf);
                dbg(DBG_PRINT, "(GRADING3A)\n");
...
{
        getdents_args_t gd_args;
        int res = copy_from_user(&gd_args, arg, sizeof(gd_args));
        dbg(DBG_PRINT, "(GRADING3A)\n");
        ...
        dirent_t d;
        int ndents = (int) gd_args.count/sizeof(dirent_t);
        int bytes = 0, nbytes = 0;

        for(int i = 0; i < ndents; i++){
                bytes = do_getdent(gd_args.fd, &d);

                if(bytes > 0){
                        res = copy_to_user(gd_args.dirp+i, &d, bytes);
